...
横並びなどのレイアウト(Flexible Box Layout)に使用するプロパティ。  
子要素の並び方を**柔軟**に指定できます。  
中身のコンテンツに合わせてサイズが伸び縮みしたり、コンテナのサイズに合わせて自動で伸縮する機能があります。  
柔軟にしたくない格子状（マス目）のレイアウトは [`Grid レイアウト`](/layout/04/) を使う方が適切です。
...
> [FLEX LAYOUT GENERATOR | フレックス レイアウト ジェネレーター](https://suiq.jp/flex-layout-generator/)  
...
> [Flexbox入門 - 横並びを実現する定番のCSS - ICS MEDIA](https://ics.media/entry/13117/)
